...
/**
 * Global(system-wide) authentication system. It loads authentication
 * information from .minecraft directory and updates them after login, so you'll
 * never need to enter your password into a custom launcher.
 */
public final class GlobalAuthenticationSystem {

	/**
	 * Performs password login with entered user details and saves session data
	 * to GlobalAuthenticationSystem
	 *
	 * @param username
	 * @param password
	 ...
	 * @throws Exception
	 *             - File I/O errors, JSON parsing errors
	 */
	 ...
	 *
	 * @param profileName
	 *            Name of profile you want to use. If there are more profiles
	 *            available, you need to specify name of profile you want to
	 *            use. If there is only one profile, API will use that profile
	 *            automatically.
	 * @return ISession which is necessary to play the game
	 * @throws Exception
	 *             File I/O, JSON parsing, Network I/O, Profile Selection errors
	 */
