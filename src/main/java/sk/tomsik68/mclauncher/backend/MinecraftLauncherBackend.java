...
/**
 * This is multi-purpose class that simplifies access to many mclauncher-api
 * components
 */
public final class MinecraftLauncherBackend {
	 ...
	 * @throws Exception
	 *             Network I/O, JSON Parsing errors
	 */
	...
	/**
	 * Returns a {@link ProcessBuilder} which has minecraft command inside of
	 * it. Use <code>ProcessBuilder.start()</code> to start the process.
	 *
	 * @param session
	 *            - Authentication session
	 * @param serverInfo
	 *            - Server to connect to. May be null.
	 * @param versionID
	 *            - Version ID to run
	 * @param launchSettings
	 *            - Launch settings(amount of RAM etc)
	 * @param moddingProfile
	 *            - Modding profile information(overriding game jar etc.). May
	 *            be null.
	 * @return ProcessBuilder which has minecraft command inside of it. No other
	 *         things are setup.
	 * @throws Exception
	 *             - Network errors, JSON parsing, process failures
	 */
	public ProcessBuilder launchMinecraft(ISession session, ServerInfo serverInfo, String versionID,
			ILaunchSettings launchSettings, IModdingProfile moddingProfile) throws Exception {
	...
	public ProcessBuilder launchMinecraft(ISession session, String versionID) throws Exception {
		return this.launchMinecraft(session, null, versionID, DEFAULT_LAUNCH_SETTINGS, null);
	}

	/**
	 * Checks if local minecraft copy needs updating. Everything is checked -
	 * libraries, assets, game jar This should be run everytime before you
	 * launch.
	 *
	 * @param versionID
	 *            - ID of version to check for update.
	 * @param progress
	 *            - {@link IProgressMonitor} in case you want to track the
	 *            update progress. It may be null
	 * @throws Exception
	 *             - Network errors, JSON parsing
	 */
