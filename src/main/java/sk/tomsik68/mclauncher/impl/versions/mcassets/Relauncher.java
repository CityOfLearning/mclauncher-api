	...
	public static void main(String[] args) {
		try {
			...
			for (int i = 0; i < args.length; ++i) {
				if ((args[i].equalsIgnoreCase("--user-name") || args[i].equalsIgnoreCase("-un"))
						&& ((i + 1) < args.length)) {
			...
			System.setProperty("net.java.games.input.librarypath",
					new File(arguments.get("-jlibpath")).getAbsolutePath());
			...
			if (arguments.containsKey("-dir")) {
				...
				for (Field field : fields) {
					// the directory is stored in field whose name is
					// obfuscated, so very little we know about it.
					// however, we know it's static and its class is File, so it
			...
			// pass all custom parameters specified in '-args' to launcher
			// component
			...
			// start the frame. if users wants to modify applet options, table
			// will be displayed before starting minecraft
