...
final class MCDownloadVersionLauncher implements IVersionLauncher {
	...
	@Override
	public List<String> getLaunchCommand(ISession session, MinecraftInstance mc, ServerInfo server, IVersion v,
		...
		for (Library lib : version.getLibraries()) {
			// each library has to be compatible, installed and allowed by
			// modding profile
			if (lib.isCompatible() && (!moddingProfileSpecified || mods.isLibraryAllowed(lib.getName()))) {
				...
				librariesString = librariesString.append(libraryProvider.getLibraryFile(lib).getAbsolutePath())
						.append(LIBRARY_SEPARATOR);
			}
		}
	...
	private String[] getMinecraftArguments(MinecraftInstance mc, File assetsDir, ISession session,
			ILaunchSettings settings, MCDownloadVersion version) {
		...
		subst.setVariable("game_directory", mc.getLocation().getAbsolutePath().replaceAll(" ", "%20"));
		subst.setVariable("game_assets", assetsDir.getAbsolutePath());
		subst.setVariable("assets_root", assetsDir.getAbsolutePath().replaceAll(" ", "%20"));
		subst.setVariable("assets_index_name", version.getAssetsIndexName().replaceAll(" ", "%20"));
		...
		for (int i = 0; i < argArray.length; i++) {
			argArray[i] = argArray[i].replaceAll("%20", " ");
		}
		return argArray;
	}
}
