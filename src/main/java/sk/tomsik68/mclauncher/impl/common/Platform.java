...
public final class Platform {
	...
	public static final IOperatingSystem[] operatingSystems = new IOperatingSystem[] { WINDOWS, MAC, SOLARIS, UNIX,
			UNKNOWN };
	...
	/**
	 * Forces current operating system. This can be used to override the default
	 * setting in case OS detection goes wrong(it doesn't go wrong very often)
	 * or for testing.
	 *
	 * @param p
	 *            - The operating system we want to use
	 */
	 ...
	 *
	 * @param name
	 *            "New" name of operating system
	 * @return IOperatingSystem which suits the criteria
	 */
	public static IOperatingSystem osByName(String name) {
		for (IOperatingSystem os : operatingSystems) {
			if (os.getMinecraftName().equalsIgnoreCase(name) || name.contains(os.getMinecraftName())
					|| os.getMinecraftName().contains(name)) {
				return os;
			}
		}
		return null;
	...
	/**
	 * Changes "old" name of operating system to "new". Old refers to names of
	 * pre-1.6 MC launcher. New refers to names of post-1.6 MC Launcher
	 *
	 * @param name
	 *            - Old name of operating system
