...
public final class YDLoginService implements ILoginService {
	...
	public IProfile createProfile(ISession session) {
		if (!(session instanceof YDSession)) {
			throw new IllegalArgumentException(
					"Profile can only be created from an YDSession. Please use YDLoginService to log in.");
		}
		return new YDAuthProfile((YDSession) session);
	}

	// performs a HTTP POST request and checks if response from the system is
	// error-less
	private YDLoginResponse doCheckedLoginPost(String url, IJSONSerializable req)
			throws YDServiceAuthenticationException {
		String jsonString = null;
		try {
			// Automatically Throws YDServiceAuthenticationException but will
			// check for IOException and convert
			jsonString = HttpUtils.doJSONAuthenticationPost(url, req);

		} catch (IOException e) {
			throw new YDServiceAuthenticationException("Failed to authenticate using Mojang authentication service.",
					e);
		}
		...
		if (response.getError() != null) {
			...
			throw new YDServiceAuthenticationException("Authentication Failed: " + response.getMessage(),
					new LoginException(
							"Error ".concat(response.getError()).concat(" : ").concat(response.getMessage())));
	...
	public ISession login(IProfile profile) throws YDServiceAuthenticationException {
		...
		if (profile instanceof LegacyProfile) {
			...
			throw new IllegalArgumentException(
					"YDLoginService can't deal with custom profile class: " + profile.getClass().getName());
