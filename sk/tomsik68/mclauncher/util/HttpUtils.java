...
public final class HttpUtils {
	...
	public static String doJSONAuthenticationPost(String url, IJSONSerializable request)
		 ...
		 * DataOutputStream will not close properley, Close will happily handle
		 * this for us
		 */
		...
		/*
		 * At this point we should be checking for a successful response code
		 * before moving forward Monjang will actually return a 403 error if a
		 * invalid username or password is given and attempting to get the
		 * InputStream will throw an IOException without knowing what caused
		 * this
		 */
	 ...
	 *            URL to request
	 * @param request
	 *            the object to send using post. It will be serialized to JSON
	 *            string
	 * @return Result of request
	 * @throws IOException
	 *             I/O Exception or HTTP errors
	 */
	 ...
	 * @param parameters
	 *            Parameters in form <code>name=Tom&amp;password=pass123</code>.
	 *            They needn't to be URL-encoded(it will be done automatically)
	 * @return The result of request
	 * @throws Exception
	 *             I/O Exception, HTTP errors or invalid key
	 */
	public static String securePostWithKey(String url, InputStream keyInput, String parameters) throws Exception {
		...
		for (byte element : serverKey) {
			if (dis.readByte() != element) {
