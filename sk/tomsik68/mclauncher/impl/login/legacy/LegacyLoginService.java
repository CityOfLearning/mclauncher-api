...
public final class LegacyLoginService implements ILoginService {
	...
	public ISession login(IProfile profile) throws Exception {
		...
		String loginResponse = HttpUtils.securePostWithKey(LOGIN_URL,
				LegacyLoginService.class.getResourceAsStream("minecraft.key"),
				"user=" + encode(profile.getName()) + "&password=" + encode(profile.getPassword()) + "&version=13");
